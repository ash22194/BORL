...
%     profile viewer;save(strcat('GridWorldWorkspace',int2str(map_size(1)),'.mat'));
    ...
    V_gptd = gptd.get_value_function([reshape(grid_x,1,map_size(1)*map_size(2));...
...
%% Run GPSARSA to build policy
    ...
    epsilon = 0.001;
    actions = [1 2 3 4];
    gpsarsa = GPSARSA(env1, actions, true, epsilon, nu, sigma_s, sigma_a, sigma0, gamma_);
    max_episode_length = 100;
    number_of_episodes = 5000;
    debug_ = true;
    gpsarsa.build_policy_monte_carlo(number_of_episodes, max_episode_length, debug_);
    V_gpsarsa = gpsarsa.get_value_function([reshape(grid_x,1,map_size(1)*map_size(2));...
                                            reshape(grid_y,1,map_size(1)*map_size(2))]);
    figure
