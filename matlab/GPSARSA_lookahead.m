classdef GPSARSA_lookahead < handle
    ...
    methods
        function gpsarsa = GPSARSA_lookahead(env, env_sim, actions, discra, epsilon, nu, sigma_s, sigma_a, sigma0, gamma_, is_reward)
            gpsarsa.env = env;
            gpsarsa.env_sim = env_sim;
        ...
        function k_ = kernel_traj_vector(gpsarsa, x)
            ...
            k_ = sparse(gpsarsa.kernel_traj(gpsarsa.D(1:size(x,1),:),repmat(x,1,size(gpsarsa.D,2))));
        end
        
        function k = kernel_traj(gpsarsa, x, y)
            if (size(x,1)==1)
                x = x';
            end
            sigma_ = repmat([gpsarsa.sigma_s; gpsarsa.sigma_a],gpsarsa.kernel_steps,1);
            k = exp(-sum(((x-y)./sigma_).^2,1)/2)';
        end
        ...
        function update(gpsarsa, xt, actiont, xt_1, actiont_1, r, gamma_)
            ...
            state_dim = (size(xt,1) + size(actiont,1));
            ...
            k_t_1 = gpsarsa.kernel_traj_vector(trajt_1);
            k_t = gpsarsa.kernel_traj_vector(trajt);
            ktt = gpsarsa.kernel_traj(trajt,trajt);
...
%             gpsarsa.C_ = gpsarsa.C_ + 1/st*(c_t*c_t');
        ...
        function build_policy_monte_carlo(gpsarsa, num_episodes, max_episode_length, epsilon_end, debug_)
            ...
            if (isempty(gpsarsa.D))
                ...
                gpsarsa.D = traj;
                gpsarsa.K_inv = 1/(gpsarsa.kernel_traj(traj, traj));
                gpsarsa.A = 1;
                gpsarsa.alpha_ = 0;
                gpsarsa.C_ = 0;
                % Original GPSARSA
