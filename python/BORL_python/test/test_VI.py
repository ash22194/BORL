...
def main():
    ...
    if (test_policies):
        ...
        dyn_sim = lambda t,s: environment.dynamics_continuous(s, policy_start_)
        int_sim = ode(dyn_sim).set_integrator('vode', method='bdf', with_jacobian=False)
    ...
    numPointsu = 121
    u_grid = np.linspace(u_limits[0], u_limits[1], numPointsu)
    states = np.mgrid[x_limits[0]:(x_grid[-1]+dx):dx, x_dot_grid[0]:(x_dot_grid[-1] + dx_dot):dx_dot]
    ...
    kernel = GPy.kern.RBF(input_dim=3, variance=1, lengthscale=np.array([1,1,1]), ARD=True)
    ...
    err = np.mean(V_pred - np.reshape(V_start, (numPointsx*numPointsx_dot,1)))
